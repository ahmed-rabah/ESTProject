...
<div class=" text-center d-flex align-items-center justify-content-center" style="height: 550px;width:500px; margin:auto;">
      ...
      <form action=" {{route('entreprise.offre.update',$offre)}}" method="post" style="border:1px solid black;border-radius:30px;background-color:whitesmoke;padding: 35px 25px;">
          <h4>modification de l'offre de stage</h4>
        ...
        <div class="form-group " style="display:none;" >
         ...
         <label>renumeration : &nbsp;</label><br>
             <div class="form-check form-check-inline pt-2">
               ...
               <input class="form-check-input" type="radio" name="renumeration" id="renumere" value="1" @if($offre->renumeration == 1) checked @endif>
                  <label class="form-check-label" for="renumere">rénuméré</label>
            </div>
            <div class="form-check form-check-inline">
                  <input class="form-check-input" type="radio" name="renumeration" id="nonrenumere" value="0" @if($offre->renumeration == 0) checked @endif>
                  <label class="form-check-label" for="nonrenumere">non rénuméré</label>
            </div>
         </div>             
        </div>
         <label for="dateDebut">date de debut de stage</label>
          <input type="date" name="dateDebut"  class="form-control" value="{{$offre->dateDebut}}">
        <span>@error('dateDebut'){{$message}} @enderror</span>
        
        <div class="form-group">
     <label for="dateFin">date de Fin de stage</label>
          <input type="date" name="dateFin"  class="form-control" value="{{$offre->dateFin}}">
        ...
        <div class="form-group" >
                            ...
                            <select class="form-select" aria-label="Default select example" id="domaine" name="domaine">
                              ...
                              <option value="{{$domaine->id}}" @if($domaine->id == $offre->domaine->id) selected @endif>{{$domaine->name}}</option>
                              @endforeach
                              </select>
                ...
                <div class="form-group" >
                            ...
                            <select class="form-select" aria-label="Default select example" id="type" name="type">
                              ...
                              <option value="{{$type->id}}" @if($type->id == $offre->type->id) selected @endif>{{$type->name}}</option>
                              @endforeach
                              </select>
                          ...
                          <label for="description">description</label><br>
                            <textarea name="description" id="" cols="50" rows="4" placeholder="description">{{$offre->description}}</textarea> 
